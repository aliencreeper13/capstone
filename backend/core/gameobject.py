"""
Base GameObject class and related utilities.
Provides foundation for all game objects with client serialization and allegiance tracking.
"""
...
class GameObject:
    """Base class for all game objects with client serialization support."""
    
    def to_client_dict(self, viewer: Optional[HasAllegianceMixin]) -> dict[str, Any]:
        """
        Convert game object to a dictionary for client transmission.
        Respects private properties and viewer authorization.
        """
        data = {}
...
class HasAllegianceMixin(ABC):
    """Mixin for objects that can have allegiance to an empire."""
    
    @public_client_property
    @abstractmethod
    def allegiance(self) -> Empire:
        """Return the empire this object is allegiant to."""
        pass